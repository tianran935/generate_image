...
DEFAULT_CATALOG_FILE = REPO_ROOT / "data_clean" / "top_50_skus_selected_categories.csv"
GRID_ROWS = 2
GRID_COLS = 4
...
def perturb_edit_attributes(
    ...
    shuffled_positions = positions_2x4()[: len(edited)]
    ...
    for item, position in zip(edited, shuffled_positions, strict=True):
        ...
        item.pop("inventory_remaining", None)

    max_badges = min(4, len(edited))
    promotion_n = promotion_count if promotion_count is not None else rng.randint(1, max_badges)
    promotion_n = min(promotion_n, len(edited))
    ...
    bestseller_n = bestseller_count if bestseller_count is not None else rng.randint(1, max_badges)
    bestseller_n = min(bestseller_n, len(edited))
